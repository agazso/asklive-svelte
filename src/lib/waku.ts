...
export async function admin(privateKeyHex: string, options: CallbackOptions | undefined = undefined): Promise<AdminSession> {
...
export async function join(secretHex: string, options: CallbackOptions | undefined = undefined): Promise<Session> {
...
export async function post(secretHex: string, message: string, waku: LightNode | undefined = undefined) {
    const secret = secp.etc.hexToBytes(secretHex)
    const topic = sha256(secret)
